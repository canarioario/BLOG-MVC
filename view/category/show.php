...
<div class="container">
    ...
    <?php foreach($data as $item) : ?>
        ...
        Artículo publicado en <?= $item->timestamp ?> en la Categoria <a class="btn btn-light" href="<?= URL ?>category/showCategory/<?= $item->category_id ?>"><?= $item->category_name; ?></a>
        </div>
        ...
        <?php if($user) : ?>
            <form class="mb-5" action="<?= URL ?>category/insertComment/<?= $getId = $item->id ?>#commentSubmitted" method="POST">
                <div class="form-group">
                    <label for="comment">Escribir un comentario</label>
    ...
    <?php if(empty($this->comments)): ?>
        <div class="card card-body bg-light mb-3">
            <p class="mb-0">Aún no se ha enviado ningún comentario.</p>
        </div>
    <?php else: ?>
        <?php foreach($this->comments as $comment) : ?>
            <div class="card card-body bg-light mb-3" id="commentSubmitted">
                <p class="mb-0"><strong class="text-primary"><?= $comment->firstname ?></strong> escribió:</p>
